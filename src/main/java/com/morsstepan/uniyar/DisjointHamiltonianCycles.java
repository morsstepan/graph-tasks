...
public class DisjointHamiltonianCycles {
	...
	public DisjointHamiltonianCycles(Graph graph) {
		this.graph = graph;
	...
	public void start() {
		...
		List<Node> auxNodes = new ArrayList<Node>(graph.nodes);
		
		// initializes the current cycle with the first node 
		cycle.add(this.graph.nodes.get(0));
		// remove it from auxNodes for simplify the search for neighbors
		auxNodes.remove(this.graph.nodes.get(0));
		
		// recursive call
		recursiveCall(this.graph.nodes.get(0), cycle, auxNodes);
	...
	public boolean recursiveCall(Node currentNode, List<Node> currentCycle, List<Node> auxNodes) {
		...
		if(currentCycle.size() == graph.nodes.size()) {
			...
			if(!currentNode.connections.contains(currentCycle.get(0))) {
				...
				// still have cycles?
				if(cycles.size() < Math.floor((graph.numberOfNodes-1)/2)) {
					
					// create new cycle and new auxNodes
					List<Node> newCycle = new ArrayList<Node>();
					List<Node> newAuxNodes = new ArrayList<Node>(graph.nodes);
					
					// initializes with the first node 
					newCycle.add(graph.nodes.get(0));
					// remove it from auxNodes
					newAuxNodes.remove(graph.nodes.get(0));
					
					boolean bool = recursiveCall(graph.nodes.get(0), newCycle, newAuxNodes);
